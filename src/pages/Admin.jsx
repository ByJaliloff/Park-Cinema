...
function Admin() {
  ...
  useEffect(() => {
    ...
    if (adminData.role !== "super_admin") {
      alert("Yalnız super_admin daxil ola bilər!");
      navigate("/admin/login");
      return;
    }
  }, [navigate]);
              ...
              <tr key={item.id} className="hover:bg-[#4A4A4A]">
                ...
                <td className="px-6 py-4">
                  {item.languages?.map((lang, i) => (
                    <span
                      key={i}
                      className="mr-1 bg-gray-700 text-white px-2 py-1 rounded text-xs"
                    >
                      {lang}
                    </span>
                  ))}
                ...
                <td className="px-6 py-4 text-right space-x-2">
                  <button
                    ...
                    className="bg-yellow-500 hover:bg-yellow-400 text-black px-3 py-1 rounded text-sm font-semibold"
                  >
                    Edit
                  </button>
                  <button
